<?php
	...
	switch($act){
		case'edit':
			...
			$arr_user=fetch_one("select * from tbl_user where pk_user_id=$id");
			$form_control="index.php?controller=edit_user&act=do_edit&id=$id";	
			break;
			case 'do_edit':
				if($_SERVER["REQUEST_METHOD"] == "POST"){
					$id=isset($_GET["id"])?$_GET["id"]:0;
					//$cc_username=htmlspecialchars($_POST["c_username"],ENT_QUOTES);
					...
					$c_email= htmlspecialchars($_POST["c_email"],ENT_QUOTES);
						...
						execute("update tbl_user set c_fullname='$c_fullname',c_address='$c_address',c_phone='$c_phone',c_password='$c_password',c_email='$c_email',c_sex='$c_sex',c_birthday='$c_birthday',c_date_created='$c_date_created' where pk_user_id=$id");
							//echo "update tbl_user set c_fullname='$c_fullname',c_address='$c_address',c_phone=$c_phone,c_password='$c_password',c_email='$c_email',c_sex='$c_sex',c_birthday='$c_birthday',c_date_created='$c_date_created' where pk_user_id=$id";
							...
							//$ckeck=fetch_one("select * from tbl_user where c_username='$c_username' and pk_user_id=$id");
						
					
				}

	}
	include_once("view/view_edit_user.php");
?>